...
function McqItem({ mcq, index }: { mcq: MCQ; index: number }) {
  return (
    <div style={{ marginBottom: '24px', fontSize: '11pt', textTransform: 'none', color: '#1e293b' }}>
      {/* Question Header with Year & Category Tag */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
        <div style={{ fontWeight: 'bold', color: '#0f172a', lineHeight: '1.5', fontSize: '11.5pt', flex: 1 }}>
          {index + 1}. {mcq.question}
        </div>
        {mcq.year && (
          <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '4px', background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', whiteSpace: 'nowrap' }}>
            📅 {mcq.year} {mcq.examCategory ? formatExamType(mcq.examCategory) : ''}
          </span>
        )}
      </div>

      {/* Options List — High contrast */}
...
export default function McqPracticeClient({ initialSubject }: { initialSubject: Subject }) {
  ...
  const shareText = encodeURIComponent(`MCQ Answers & Solutions — ${cleanTitle} (${subject.code}), ${semName} | TU Notes Hub`)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 64px)', background: '#0b0f19', position: 'relative' }}>
      ...
      <div className="mcq-page-grid" style={{ flex: 1, display: 'grid', gap: '24px', padding: '16px 24px', maxWidth: '1400px', margin: '0 auto', width: '100%', gridTemplateColumns: 'minmax(0, 1fr) 340px', alignItems: 'stretch' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Header Card (Rich SEO Information & Badges) */}
          <div className="glass-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', background: 'rgba(255,255,255,0.02)' }}>
            
            {/* Breadcrumbs / Badges */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <span className="badge badge-semester" style={{ fontSize: '11px', fontWeight: 700 }}>
                🎓 {facultyName}
              </span>
              <span className="badge badge-primary" style={{ fontSize: '11px', fontWeight: 700, background: 'rgba(99,102,241,0.15)', color: '#a5b4fc', border: '1px solid rgba(99,102,241,0.3)' }}>
                📚 {semName}
              </span>
              {years.length > 0 && (
                <span className="badge" style={{ fontSize: '11px', fontWeight: 700, background: 'rgba(16,185,129,0.15)', color: '#34d399', border: '1px solid rgba(16,185,129,0.3)' }}>
                  📅 Years: {years.join(', ')}
                </span>
              )}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
              <div>
                <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--clr-text-1)', margin: '0 0 6px 0', lineHeight: 1.3 }}>
                  {cleanTitle} ({subject.code}) — {semName} MCQs & Solutions
                </h1>
                <p style={{ fontSize: '13px', color: 'var(--clr-text-2)', margin: 0 }}>
                  Official Tribhuvan University (TU) past paper multiple choice questions with verified answers.
                </p>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <button
                  ...
                  style={{ padding: '10px 20px', fontSize: '13px', fontWeight: 700, borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                >
                  <span>⬇️</span> Download MCQ PDF
                </button>
              </div>
            </div>
          ...
          <div style={{
            ...
            <div className="mcq-paper-sheet" style={{
              ...
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                ...
                <h2 style={{ fontSize: '12pt', fontWeight: 'bold', margin: '0 0 4px 0', color: '#1e293b' }}>
                  {facultyName} — {semName}
                </h2>
                <h3 style={{ fontSize: '13pt', fontWeight: 'bold', margin: '0 0 8px 0' }}>
                  OFFICE OF THE DEAN
                </h3>
                <div style={{ fontSize: '12pt', fontWeight: 'bold', margin: '0 0 15px 0', color: '#0f172a' }}>
                  Board Exam MCQs ({yearText})
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11pt', marginBottom: '4px', fontWeight: 'bold' }}>
                <div>
                  <div style={{ marginBottom: '3px' }}>Program: {facultyName}</div>
              ...
              <div style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '12pt', marginTop: '10px', marginBottom: '15px' }}>
                Group A (Multiple Choice Questions) — {semName}
              </div>

              <div style={{ fontWeight: 'bold', fontSize: '11pt', marginBottom: '15px', color: '#1e293b' }}>
                Attempt all questions. Correct answers are highlighted in green with detailed explanations.
              </div>
        ...
        <div className="mcq-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="glass-card" style={{ padding: '24px', background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(168,85,247,0.08))', border: '1px solid rgba(99,102,241,0.2)' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--clr-text-3)', marginBottom: '16px', lineHeight: 1.5 }}>
              Get instant direct downloads, unlock full AI exam predictor models, and practice all past paper MCQs offline.
            </p>
      ...
      {/* 15-SECOND SPONSORED AD COUNTDOWN MODAL (FOR FREE USERS) */}
      {downloadAdActive && (
        <div style={{
          position: 'fixed',
          inset: 0,
