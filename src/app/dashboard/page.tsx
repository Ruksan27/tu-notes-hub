...
function AICompareTool({ subjects, isElite }: { subjects: Subject[]; isElite: boolean }) {
  ...
  async function handleImportMcqs() {
    ...
    setShowExpl({})
    try {
      const res = await fetch(`/api/mcq/${selectedSubjectId}`)
      const data = await res.json()
      if (res.ok && data.mcqs && data.mcqs.length > 0) {
        setMcqs(data.mcqs)
        setMcqDisplayTitle(`${currentSubject?.title || 'Subject'} — Question Bank MCQs`)
        setMcqFromHistory(true)
        toast.success(`📥 Loaded ${data.mcqs.length} MCQs from Question Bank! 🎉`)
      } else {
        toast.info('No saved MCQs found in database for this subject yet. Select 2+ past papers to generate new ones!')
      }
    } catch {
    ...
    <div className="flex flex-col gap-6">
      ...
      {!report && (
        <div className="glass-card p-6 sm:p-8 space-y-6 max-w-4xl mx-auto">
          ...
          {/* Action buttons */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 border-t border-[var(--clr-border)]">
            <button
              onClick={runAIAnalysis}
              disabled={loading || selectedPaperIds.length < 2}
              className="btn btn-primary font-bold justify-center py-3"
            >
              {loading ? (
                ...
                <span className="flex items-center gap-2">
                  <Brain className="w-5 h-5" /> Predict Patterns
                </span>
              )}
            </button>
            ...
            <button
              ...
              className="btn btn-outline font-bold justify-center py-3"
            >
              {generatingMcqs ? (
                <span className="flex items-center gap-2">
            ...
            <button
              onClick={handleImportMcqs}
              disabled={loadingImport || !selectedSubjectId}
              className="btn font-bold justify-center py-3 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 transition-all"
            >
              {loadingImport ? (
                <span className="flex items-center gap-2">
                  <span className="animate-spin text-lg">⚙️</span> Loading...
                </span>
              ) : (
                <span className="flex items-center gap-2">
                  📥 Import Bank MCQs
      ...
      {/* MCQs View */}
      {mcqs && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" id="ai-mcq-container" style={{ padding: '28px', marginBottom: '24px' }}>
          <div className="flex items-center justify-between flex-wrap gap-4 mb-6 pb-4 border-b border-white/10">
            <div>
              <div className="flex items-center gap-2.5">
                <span className="text-2xl">📝</span>
                <h3 className="text-xl font-black text-white">{mcqDisplayTitle || currentSubject?.title || 'Subject'}</h3>
              </div>
              <p style={{ fontSize: '13px', color: 'var(--clr-text-3)', marginTop: '4px' }} className="flex items-center gap-2 font-medium">
                <span>{mcqs.length} Total Questions</span>
                <span>•</span>
                <span className="text-indigo-400">{mcqFromHistory ? 'Loaded from Question Bank / History' : 'Freshly Generated'}</span>
              </p>
            </div>

            <div className="flex items-center gap-2 flex-wrap hide-on-print">
              {Object.keys(userAnswers).length > 0 && (
                <div className="px-3 py-1.5 rounded-lg bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 font-bold text-xs">
                  🎯 Score: {Object.entries(userAnswers).filter(([idx, ans]) => mcqs[Number(idx)]?.correctOption === ans).length} / {mcqs.length}
                </div>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {mcqs.map((m: any, i: number) => {
              ...
              const isCorrect = userAns === m.correctOption

              return (
                <div key={i} className="mcq-item transition-all" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', padding: '20px', borderRadius: '16px' }}>
                  <div className="flex items-start gap-3 mb-4">
                    <span className="shrink-0 w-7 h-7 rounded-lg bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 font-black text-xs flex items-center justify-center">
                      Q{i + 1}
                    </span>
                    <p className="mcq-question font-bold text-white text-base leading-snug" style={{ margin: 0 }}>
                      {m.question}
                    </p>
                  </div>

                  <div className="mcq-options grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {m.options.map((opt: string, idx: number) => {
                      const letter = String.fromCharCode(65 + idx)
                      const isOptionSelected = userAns === idx
                      const isOptionCorrect = m.correctOption === idx

                      let bg = 'rgba(255,255,255,0.03)'
                      let border = '1px solid rgba(255,255,255,0.08)'
                      let text = 'var(--clr-text-2)'
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setUserAnswers(prev => ({ ...prev, [i]: idx }))
                          }}
                          className="mcq-option text-left cursor-pointer transition-all duration-200 hover:scale-[1.01]"
                          style={{
                            padding: '12px 16px',
                            borderRadius: '10px',
                            background: bg,
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <span className="w-6 h-6 rounded-md bg-white/5 border border-white/10 flex items-center justify-center text-xs font-bold shrink-0">
                              {letter}
                            </span>
                            <span>{opt}</span>
                          </div>
                          {isAnswered && isOptionCorrect && <span className="font-black text-emerald-400 text-sm">✓</span>}
                          {isAnswered && isOptionSelected && !isCorrect && <span className="font-black text-rose-400 text-sm">✗</span>}
                        </button>
                      )
                    })}
                  </div>
                  ...
                  {m.explanation && (
                    <div className="mt-3">
                      <button
                        type="button"
                        onClick={() => setShowExpl(prev => ({ ...prev, [i]: !prev[i] }))}
                        className="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors flex items-center gap-1.5 mt-2"
                      >
                        💡 {showExpl[i] ? 'Hide Explanation' : 'Show Explanation'}
                      </button>
                      {showExpl[i] && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-2 p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-xs text-indigo-200 font-medium">
                          <strong className="text-indigo-300">Explanation:</strong> {m.explanation}
                        </motion.div>
                      )}
                    </div>
                  )}
      ...
      {showHistoryModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 md:p-6 overflow-y-auto">
          <motion.div 
            initial={{ opacity: 0, scale: 0.94, y: 10 }} 
            animate={{ opacity: 1, scale: 1, y: 0 }} 
            exit={{ opacity: 0, scale: 0.94, y: 10 }}
            transition={{ duration: 0.2 }}
            className="bg-[#090d16]/95 border border-indigo-500/25 rounded-3xl max-w-3xl w-full p-6 md:p-8 shadow-[0_0_80px_rgba(99,102,241,0.2)] relative space-y-6"
          >
            {/* Modal Header */}
            <div className="flex items-start justify-between pb-5 border-b border-white/10">
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 via-indigo-500 to-cyan-400 flex items-center justify-center text-2xl shadow-lg shadow-indigo-500/30 border border-white/20">
                  📜
                </div>
                <div>
                  <h3 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
                    Saved AI History
                    <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-indigo-500/15 text-indigo-300 border border-indigo-500/30">7 Days Auto-Sync</span>
                  </h3>
                  <p className="text-sm text-slate-400 mt-0.5">Generated predictions & MCQs are automatically saved for 7 days before auto-cleanup.</p>
                </div>
              </div>
              <button 
                onClick={() => setShowHistoryModal(false)} 
                className="p-2 rounded-xl bg-white/5 hover:bg-white/15 border border-white/10 text-slate-400 hover:text-white transition cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {loadingHistory ? (
              <div className="py-16 text-center text-slate-400 space-y-3">
                ...
                <p className="text-base font-semibold text-slate-300">Loading your saved history...</p>
              </div>
            ) : historyList.length === 0 ? (
              <div className="py-16 text-center text-slate-400 space-y-3">
                <p className="text-4xl">📭</p>
                <p className="text-base font-bold text-slate-200">No saved AI history found.</p>
                <p className="text-sm text-slate-400 max-w-md mx-auto leading-relaxed">Generate AI Exam Predictions or MCQs — they will be automatically saved here for 7 days so you can reload or study anytime!</p>
              </div>
            ) : (
              <div className="space-y-3.5 max-h-[60vh] overflow-y-auto pr-1">
                {historyList.map((item) => {
                  ...
                  return (
                    <div 
                      key={item.id} 
                      className="p-5 rounded-2xl bg-slate-900/70 border border-white/10 hover:border-indigo-500/40 hover:bg-slate-900/90 transition-all duration-200 shadow-md group"
                    >
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="space-y-2 flex-1 min-w-[220px]">
                          {/* Type badge + expiry */}
                          <div className="flex items-center gap-2.5 flex-wrap">
                            <span className={`text-xs font-bold px-3 py-1 rounded-full ${item.type === 'EXAM_REPORT' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'}`}>
                              {item.type === 'EXAM_REPORT' ? '📊 EXAM PREDICTION' : `🎯 MCQ SET${mcqCount > 0 ? ` (${mcqCount} MCQs)` : ''}`}
                            </span>
                            <span className="text-xs font-semibold text-amber-300 bg-amber-500/15 px-3 py-1 rounded-full border border-amber-500/30 flex items-center gap-1.5">
                              <Clock className="w-3.5 h-3.5" /> {item.daysRemaining === 1 ? 'Expires tomorrow' : `${item.daysRemaining} days left`}
                            </span>
                          </div>
                          {/* Subject name */}
                          <h4 className="font-extrabold text-base text-white group-hover:text-indigo-200 transition-colors tracking-wide">
                            {item.subjectTitle}
                          </h4>
                          {/* Year pills */}
                          {years.length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                              ...
                              {years.map((yr) => (
                                <span key={yr} className="text-xs font-extrabold px-2.5 py-0.5 rounded-lg bg-indigo-500/15 text-indigo-300 border border-indigo-500/30">
                                  {yr}
                                </span>
                              ))}
                            </div>
                          ...
                          <p className="text-xs text-slate-400 font-medium">
                            Saved on {new Date(item.createdAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-2.5">
                          <button 
                            ...
                            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 via-indigo-500 to-cyan-500 text-white text-xs font-extrabold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2 cursor-pointer"
                          >
                            <span>👁️</span> Load & View
                          </button>
                          <button 
                            onClick={(e) => deleteHistoryItem(item.id, e)} 
                            className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/25 hover:text-rose-300 hover:scale-105 transition-all cursor-pointer" 
