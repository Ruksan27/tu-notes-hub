...
export default function ProjectDetailClient({ project }: { project: Project }) {
  ...
  useEffect(() => {
    ...
    // Fetch reviews
    fetch(`/api/projects/reviews?projectId=${project.id}`)
      .then(r => r.json())
      .then(d => {
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/projects/reviews', {
        ...
        body: JSON.stringify({ projectId: project.id, rating: reviewRating, comment: reviewComment })
      })
      const data = await res.json()
      if (res.ok && data.review) {
        toast.success('Review posted successfully! 🎉')
        const user = JSON.parse(storedUserStr)
        const newReview = { ...data.review, user: { name: user.name } }
  ...
  const handleDeleteReview = async (reviewId: string) => {
    ...
    try {
      const res = await fetch(`/api/projects/reviews?reviewId=${reviewId}&projectId=${project.id}`, { method: 'DELETE' })
      if (res.ok) {
